...
void seg_display_2digits_Hex(int16_t value)
{
  int16_t dd = value%256;
  close_seven_segment();
  show_seven_segment_Hex(3,dd/16);
  DrvSYS_Delay(scanDelay);

  close_seven_segment();
  show_seven_segment_Hex(2,dd%16);
  DrvSYS_Delay(scanDelay);
}

void seg_display_2digits(int16_t value)
{
  int16_t dd = value%100;
  close_seven_segment();
  show_seven_segment_Hex(1,dd/10);
  DrvSYS_Delay(scanDelay);

  close_seven_segment();
  show_seven_segment_Hex(0,dd%10);
  DrvSYS_Delay(scanDelay);
}
...
int main(void)
{
    ...
    seg_display_2digits_Hex(Timer0Counter);
    seg_display_2digits(Timer0Counter);
	}
}
