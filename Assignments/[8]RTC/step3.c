...
//step 3a : show Timer 0 on 7 segment i2c
//step 3b : show Timer 0 on 7 segment(3,2) on board
set - CLR on line 63
		- TLR on line 64
		- CAR on line 68
		- TAR on line 69
...
void seg_display(int32_t clock) {

	//digit = value / 1000;
	close_seven_segment();
	//show_seven_segment(3,clock >> 20 & 0xF);
	DrvSYS_Delay(scanDelay);

	//value = value - digit * 1000;
//	digit = value / 100;
	close_seven_segment();
	//show_seven_segment(2,clock >> 16 & 0xF);
	DrvSYS_Delay(scanDelay);

	//value = value - digit * 100;
//	digit = value / 10;
	close_seven_segment();
	show_seven_segment(1,clock >> 12 & 0xF);
	DrvSYS_Delay(scanDelay);

//	value = value - digit * 10;
//digit = value;
	close_seven_segment();
	show_seven_segment(0,clock >> 8 & 0xF);
	DrvSYS_Delay(scanDelay);
	}
...
void seg_display_1(int16_t value) {
  int8_t digit;
	digit = value / 1000;
	//close_seven_segment();
...
void RTC_IRQHandler(void) { 	// default every 1 s.
	...
	if (inpw(&RTC->RIIR) & 0x2) {	// TIF = 1?
		...
		//Write_to_any8574(0x70, HEX2Disp(clock & 0xF));
    //Write_to_any8574(0x72, HEX2Disp(clock >> 4 & 0xF));
...
void TMR0_IRQHandler(void) {	// Timer0 interrupt subroutine
	...
	print_lcd(3, lcd_line2);
 	TIMER0->TISR.TIF = 1; 	   // Write 1 to clear TIF
	//Write_to_any8574(0x70, HEX2Disp(TimerCounter));
  //Write_to_any8574(0x72, HEX2Disp(TimerCounter >> 4));
...
int32_t main (void) {
	...
	while (Alarm_E) {
		...
		seg_display(clock);
		seg_display_1(TimerCounter);
		LOCKREG();
	}
	while (1) {
		//clk = inpw(&RTC->TLR) & 0xFFFFFF;
